...
- **Native Game Convertor tool**: `gconv` — a standalone game asset converter matching `gconv.exe` from the IGI 2 Editor — now lives in its own repo at [project-igi-gconv](https://github.com/jones-hm/project-igi-gconv). The editor bundles the prebuilt binary at `editor/tools/gconv.exe`.
